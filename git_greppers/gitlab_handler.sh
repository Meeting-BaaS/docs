...
fetch_gitlab_mr_comments() {
  ...
  if [[ -z "$mr_number" ]]; then
    debug 2 "MR number is empty in fetch_gitlab_mr_comments" >&2
    echo ""
    return
  fi
  
  # Get MR details using GitLab API (more reliable than glab mr view)
  if command -v jq &> /dev/null; then
    debug 3 "Using GitLab API to fetch MR details" >&2
    local project_path=${repo_identifier/\//%2F}
    local mr_details=$(glab api "projects/$project_path/merge_requests/$mr_number" 2>/dev/null)
    
    if [[ -n "$mr_details" ]]; then
      # Extract MR description
      local mr_description=$(echo "$mr_details" | jq -r '.description')
      if [[ -n "$mr_description" && "$mr_description" != "null" ]]; then
        comments+="MR DESCRIPTION:\n$mr_description\n\n"
        has_comments=true
        debug 3 "Added MR description to comments" >&2
      fi
      
      # Get MR comments using API
      debug 3 "Fetching MR comments using API" >&2
      local mr_notes=$(glab api "projects/$project_path/merge_requests/$mr_number/notes" 2>/dev/null)
      
      # Extract comments
      local mr_comments=$(echo "$mr_notes" | jq -r '.[] | "Author: \(.author.name)\nDate: \(.created_at)\n\(.body)\n"')
      if [[ -n "$mr_comments" && "$mr_comments" != "null" ]]; then
    ...
    debug 2 "jq not available, falling back to basic comment extraction" >&2
    ...
    if [[ -n "$mr_details" ]]; then
      ...
      # Extract comments from mr view output
      local mr_comments=$(echo "$mr_details" | sed -n '/^Comments:/,/^$/p' | grep -v "^Comments:")
