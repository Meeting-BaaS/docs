...
# Use REPO_FLAGS if set, otherwise default to '--with-diff --include-code --only-with-pr-mr'
: "${REPO_FLAGS:=--with-diff --include-code --only-with-pr-mr}"
...
if [ -d "$REPO_PATH" ]; then
    ...
    ./git_grepper.sh "$REPO_PATH" "$DEBUG_LEVEL" $REPO_FLAGS
