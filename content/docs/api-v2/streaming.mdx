...
To timestamp the audio stream: `audio_time = handshake_receipt_time + offset + cumulative_samples / sample_rate`. Audio capture starts when the bot opens the meeting page, which is later than `joined_at` in the bot details. On reconnection the bot sends a new handshake with `offset` reset to `0.0`, so restart the clock; audio during the disconnection is dropped.
