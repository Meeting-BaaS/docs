...
<Cards>
  ...
  <Card title="Zoom Integration" href="/docs/api-v2/getting-started/zoom">
...
|---|---|---|---|
| Config object | `meet_config` | `teams_config` | `zoom_config` |
| Sign-in mechanism | SAML SSO (Meeting BaaS is the IdP) | Username + password on `login.microsoftonline.com` | Stored Zoom credential + OBF/ZAK tokens |
| Parent resource | Meet Workspace (domain + SAML keypair) | Teams Workspace (M365 domain, no keypair) | Zoom credential |
| Per-user identity | Meet Login (`email`) | Teams Login (`email` + `password`) | — |
| One-time infra setup | Configure a Legacy SSO profile in Google Admin Console; upload the signing certificate | Provision an **MFA-free** M365 account; no IdP/cert setup | Store the Zoom credential |
| Round-robin pools | `email_group` | `email_group` | — |

## Shared concepts

Both Google Meet and Microsoft Teams authentication share the same operational model:
